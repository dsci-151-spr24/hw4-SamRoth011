...
gss16 %>%
  mutate(
    advfront = fct_collapse(
      advfront,
      agree = c("Strongly agree", "Agree"),
      `Not agree` = c("Don't know", "Disagree", "Strongly Disagree")),
    advfront = fct_relevel(advfront, "agree", "Not agree")
  )

advfront_counts <- gss16 %>%
  count(advfront) %>%
  print(advfront_counts)
```
...
polviews_counts <- gss16 %>%
  count(polviews) %>%
  print(polviews_counts)
```
...
# gss16_fit_rs_1 <- gss16_wflow_1 %>%
  # fit_resamples(resamples = gss16_folds)
...
***I couldn't get either to work, with the error "Can't select columns that don't exist.
               ✖ Column `.pred_Dont know` doesn't exist."
...
***You can simulate the distribution of the proportion of people that were harassed.**
...
```
***There is a 95% chance that the proportion of Americans who were harassed is between 15 and 19 percent.***
...
***90% confidence would be narrower as it would need to be less accurate, and therefore could be more precise.***
